...
int check_avg(chunk_list* chunks, ping_time_chunk* ping);
int check_avg_interval(chunk_list* chunks, ping_time_chunk* ping);
int check_avg_gradient(chunk_list* chunks, ping_time_chunk* ping);
int check_avg(chunk_list* chunks, ping_time_chunk* ping);
...
void report(chunk_list* chunks, char* out_range_stats, ping_time_chunk* ping) {
	...
	if(!strncmp(out_range_stats, "avg", strlen(out_range_stats))) {
		...
		ping_time_chunk* ptr = lasts->head;
		while(ptr->next != NULL) {
